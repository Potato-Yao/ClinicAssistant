...
喜讯，在github上找到了一个叫fan-control的rust项目，它为了调用LibreHardwareMonitor，自己用C#写了一个wrapper，是通过tcp来实现通信的，然后在rust里调用。我会rust，把相关rust代码翻译成Java应该就能拿到数据了。
...
比如HardwareInfoManager，需要手动调用update()方法来更新数据，这其实很不符合逻辑，因为在调用获取信息的方法时，思维上就暗含着数据是新的。所以写了个线程，在manager创建出来后就固定时间调用update()
...
还写了获得cpu频率的方法。wrapper获得的是每个核心的频率，但是最后只能输出一个总的频率，我希望这个总的频率能和任务管理里给出的那个一致或者接近。我最开始使用频率中最大值输出，发现这样和任务管理器的频率相比高很多，说明微软不是这么算的。于是我取数值最大的三个点，取加权平均。坏消息是一直没找到能匹配的的权重。

但是好消息是我这学期学了数值分析。于是我记录下取0.5 0.25 0.25权重的值与同时任务管理器的值，取了四次做牛顿插值，最后得到一个三次多项式。这次拟合效果就好多了，但是仍旧有点偏高，个人认为第一是我权重取的不对，第二是四个点太少，因为我只关注`[0, 5]`这个区间的值，所以多项式次数高一点问题不大。第三是我采集数据的方法（把我的程序和任务管理器放一起截图）太粗糙，这二者表示的数据可能并不是同时的，造成了不小的测量误差。

## 12.19 凌晨

回看日志，12.4那天晚上我写了一个生成代码用的python脚本。当时图省事直接用的元组列表，现在报应来了。

元组中有一项（假设是A）用来生成类似于`cpu.A(lhmHelper.getValue(index[5]));`的代码，但是我现在需要的是`cpu.A(index[5]);`，这就没办法了。所以我只好把A写成`A(index[5]); //`，这样生成的代码就是`cpu.A(index[5]); //(lhmHelper.getValue(index[5]));`，就能正常用了，跟SQL注入似的。

## 12.21 晚上

这几天写了javadoc，改了之前因为急匆匆写代码造成的一些设计纰漏和bug，写了一个csvUtil，这样就可以记录每次烤机的数据了

## 12.23 上午

写了检测目录下的CLINC_OP中的工具
